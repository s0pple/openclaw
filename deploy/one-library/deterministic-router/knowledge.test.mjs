...
assert.equal(isKnowledgeContentQuestion("And what powers does he have?", { hasKnowledgeContext: true }), true);
assert.equal(extractKnowledgeTitle("Who is Sunny in Shadow Slave?"), "Shadow Slave");
assert.equal(
  extractKnowledgeTitle("Was weiß ich bis zu meinem aktuellen Lesestand über Sunny in Shadow Slave?"),
  "Shadow Slave",
);
assert.equal(extractKnowledgeTitle("Was passiert in Shadow Slave in Kapitel 2200?"), "Shadow Slave");
...
for (const status of [
  ...
  assert.match(renderKnowledgeToolResult({
    content: [{ type: "text", text: JSON.stringify({ status }) }],
  }), new RegExp(`status: ${status}`));
}

assert.match(renderKnowledgeToolResult({
  content: [{ type: "text", text: JSON.stringify({ status: "answered", answer: "unsafe", citations: [] }) }],
}), /citation_validation_failed/);
