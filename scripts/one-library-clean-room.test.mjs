  ...
  return new Promise((resolve, reject) => {
    ...
    gateway.on("close", (code, signal) => {
      if (signal === "SIGTERM" || signal === "SIGKILL") finish();
        ...
        finish(new Error(`clean-room gateway exited unexpectedly (${code ?? signal}): ${output}`));
    });
...
try {
  ...
  const recoveredPlugins = JSON.parse(
    execFileSync("openclaw", ["plugins", "list", "--json"], {
      cwd: repoRoot,
      env: openclawEnv,
      encoding: "utf8",
  ...
  const recoveredDeterministic = recoveredPlugins.plugins.find(
    (plugin) => plugin.id === "deterministic-router",
  );
